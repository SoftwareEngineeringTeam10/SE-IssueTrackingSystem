...
public class StatisticsService {
    ...
    // 일별 이슈 등록 수
    public Map<String, Integer> getDailyStats(int projectId) {
    ...
    // 월별 이슈 등록 수
    public Map<String, Integer> getMonthlyStats(int projectId) {
        ...
        for (Issue issue : issues) {
            ...
            monthlyMap.put(
                    month,
                    monthlyMap.getOrDefault(month, 0) + 1
            );
        }
    ...
    // 상태별 이슈 수
    public Map<IssueStatus, Integer> getStatusStats(int projectId) {
    ...
    // 우선순위별 이슈 수
    public Map<Priority, Integer> getPriorityStats(int projectId) {
        ...
        for (Issue issue : issues) {
            ...
            if (issue.priority != null) {
                ...
                priorityMap.put(
                        issue.priority,
                        priorityMap.get(issue.priority) + 1
                );
            }
        }
    ...
    // 전체 통계 출력
    public void printAllStats(User currentUser, int projectId) {
        ...
        for (Map.Entry<IssueStatus, Integer> e :
                getStatusStats(projectId).entrySet()) {

            System.out.println(
                    "  "
                            + String.format("%-10s", e.getKey())
                            + " : "
                            + buildBar(e.getValue())
                            + " (" + e.getValue() + "건)"
            );
        ...
        for (Map.Entry<Priority, Integer> e :
                getPriorityStats(projectId).entrySet()) {
