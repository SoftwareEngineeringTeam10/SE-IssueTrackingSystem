...
- `priority`는 `{BLOCKER, CRITICAL, MAJOR, MINOR, TRIVIAL}` 중 하나이다.

## Postconditions

- 새 `Issue` 인스턴스 `i`가 생성되었다. *(instance creation)*
- `i.title := title`, `i.description := description`, `i.priority := priority`로 설정되었다. *(attribute modification)*
...
- `priority` 기본값(MAJOR) 처리는 UI에서 채워 보내는 것으로 가정하여 Precondition에 강하게 명시한다.
